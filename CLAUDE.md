...
| `/app`, `/app/orders`, `/app/orders/[id]`, `/app/inquiries`, `/app/inquiries/[id]`, `/app/performances`, `/app/performances/[id]`, `/app/members`, `/app/members/[id]`, `/app/leaderboard`, `/app/scan`, `/app/sell`, `/app/statement`, `/app/comp`, `/app/users`, `/app/users/[id]`, `/app/stats`, `/app/more`, `/app/account`, `/app/notifications`, `/app/welcome`, `/app/install`, `/app/session`, `/app/login`, `/app/forgot`, `/app/authorize`, `/app/join/[code]` | `src/app/app/…` (own route group + layout) | **Cecilija** (the app's name since #489; "moreškant" now names only the dancer and the roster section), Croatian only, noindex, outside `(frontend)`/`(payload)`. **The shell is #495:** the permission → screen table is `src/lib/app/screens.ts` and NOTHING re-types it — the phone bar, the laptop sidebar, the active-tab highlighter and every page gate (`openScreen()` in `src/app/app/gate.tsx`) read it, and a screen ticket's only change there is moving its permission from `unlockedBy` into `servesToday`. `/app` itself is a 307 to the person's first tab; access is "the set unlocks at least one screen" (`src/lib/app/access.ts`); attendance answers write through `POST /api/app/attendance`; `/app/performances` is **Izvedbe**, one screen whose content is decided by `can()`: the dancer's agenda and answers (#457), the voditelj's Dodaj / Uredi / Otkaži / Pragovi on a NON-public row (#503), and the blagajna's half (#502) — sold of capacity and the channel split on every public row, the per-show numbers the old `/admin/stats/[id]` drill-down had (revenue only for `finance`), and six named actions, five of which reuse their existing route unchanged (`/api/shows/[id]/{cancel,reschedule,move-to-indoor,offline-sales}` and the Narudžbe link) while `POST /api/app/performances/[id]/pause` is the one that had none; the two shared routes `/api/app/performances` and `…/[id]` decide which half may write from the ROW rather than the URL, and share their validator and their writer with the MCP `create_performances` tool; a dancer's login is issued by the invitation (`POST /api/app/invite`), never self-registered; `/app/authorize` is the MCP connector's consent screen (#438); `/app/welcome` is the once-per-device walkthrough (#457, glossary *Dobrodošlica*, *Ljestvica*); `/app/install` is the install guide and one of the two `/app` pages outside the access decision, because it is the QR target for a rehearsal (#455); `/app/scan` is **Skener**, the door's screen (#504): the camera, the four result states, the manual-admit search and the "ušlo X od Y" ring, with `/admin/scan` 308ing to it; `/app/sell` and `/app/statement` are **Prodaja** and **Obračun** (#505), the reseller's two screens, a port of the Backoffice partner dashboard onto the five existing `/api/partner/*` routes (sell, cancel, cancel/undo, sales, reconciliation), which stay as they are — a partner login with no Partner link or a deactivated one gets a sentence and no figure, and Partners CRUD plus partner logins stay in the Backoffice; `/app/stats` is **Statistika** (#508), the season in counts for `tickets`, `season_stats` and `finance`: the season band, one row per public izvedba with its channel split, the trajectory and channel-mix charts moved as-is from the Backoffice dashboard, and "gratis po članu" (#506's own tally, read through `repo.comp`) for `tickets` only — **never money on this screen for anybody**, because euros are Financije (#509, decided in #500), and a row links into `/app/performances/[id]` only for a reader who unlocks Izvedbe, so the shared `member` login (`season_stats`, its only screen) reads the numbers with no link that would refuse itself; `/admin/stats` and `/admin/stats/[showId]` 308 to `/app/stats` and `/app/performances/[showId]` through `src/lib/backoffice-ports.ts`, which is also where #504's `/admin/scan` entry now lives; `/app/orders` is **Narudžbe**, the blagajna's order list and one order (#501): search, a performance and a state filter and a pager, all in the query string, and four named actions on the detail — Povrat (`refunds` only), Pošalji ulaznice ponovno, Otvori PDF and Uredi kupca, whose `PATCH /api/app/orders/[id]/buyer` is the screen's one write; `/app/inquiries` is **Upiti**, the enquiry inbox (#507): unanswered first with booking enquiries lifted above general ones, a `state` filter and a pager in the query string, the whole message on the detail, **Odgovori** as a server-built `mailto:` (Cecilija never sends the mail; Tatjana answers from `info@` in Gmail) and **Označi riješenim** through `POST /api/app/inquiries/[id]/handled`, a switch rather than a toggle so the undo is the same button and a retry is harmless; `/app/comp` is **Gratis** (#506), where a `tickets` holder hands a society member free seats: Podijeli gratis (a required member picker with an inline Dodaj člana, the two steppers, a prefilled holder name) on the existing `POST /api/comp/issue`, Poništi gratis on a whole order or one ticket on the existing `POST /api/comp/cancel` behind a confirmation (that route has no undo, so none is offered), and the Gratis po članu season table — promo codes stay in the Backoffice; `/app/members` is **Članovi** (#511), the voditelj's roster screen, which absorbed Pozivnice: the rehearsal join code and its pending claims, Dodaj plesača, and a searchable list (diacritic-insensitive, active moreškanti first) whose every row carries the two invitation channels and links to the profile at `/app/members/[id]`, where nadimak, mobitel, e-mail, plesne uloge, glavna uloga and aktivan are written through `PATCH /api/app/members/[id]` and a new dancer through `POST /api/app/members` — both guarded by `requirePermission(req,'moreska')`, both running the Members `beforeValidate` hook through the seam so nickname uniqueness stays the collection's, and `name`/`note` refused in the route because they are the ADR-0019 attribution half; `/app/invitations` and `/app/pozivnice` both 308 there, and deleting a member stays in the Backoffice; `/app/users` is **Korisnici** (#510), where a `users` holder reads every account as Croatian permission chips and changes it through five named actions — Dozvole, Resetiraj lozinku, Poveži partnera, Poveži člana, Dijeljeni račun — plus Novi korisnik on the list. Its five routes ARE the lock, because `permissions`, `shared` and `member` are field-locked and the seam writes with `overrideAccess`: each re-checks `users` in the handler, refuses the caller taking `users` off their own row (409), refuses a `shared` login editing itself (403) and refuses a named-person set on an address-less account (400). A new or reset login is handed over one of two ways and never a third: a one-hour sign-in link when there is an address, a temporary password shown once when there is none. Linking a member carries #487's `taken` / `not-active` refusals and writes the link only; deleting an account stays Backoffice work; `/app/account` merges the old password and self-link screens, carries the push on/off switch, and is where a voditelj who dances links their own Member, the one sanctioned way past the `Users.member` field lock (#462); `/app/notifications` is the *Sandučić obavijesti* behind the header bell (#496), a Više row rather than a tab, open to every account the access decision admits; **since #463 an invitation LINK is what signs a dancer in** (`/app/session?token=…` → `POST /api/app/session`), a voditelj hands invitations over by SMS from `/app/members`, and `/app/join/[code]` is the rehearsal QR — the other page outside the access decision, public because the person scanning has no login yet, and the only public page that WRITES (a claim a voditelj approves); see `docs/agents/moreskant-app.md`. **The Croatian segments were renamed by #495**, each with a 308 in `next.config.ts`; two of them (`instalacija`, `prijava`) are permanent because a QR on a wall and an SMS link cannot be edited. Shape and scope of the app: [ADR-0027](docs/adr/0027-cecilija-one-staff-app-payload-backoffice.md) |

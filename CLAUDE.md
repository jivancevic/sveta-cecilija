...
2. **Regenerate `package-lock.json` in a Linux container before any merge that touches deps. Hard pre-merge gate, not a tip.** `npm install` on macOS resolves transitives differently and can omit entries Linux `npm ci` requires (esbuild platform binaries lose their `optional: true` flag; transitive optional peers like `yaml@2.9.0` get dropped). Workflow: `rm -rf node_modules package-lock.json && docker run --rm -v "$PWD":/app -w /app node:22 sh -c "npm install"` to generate the lockfile in Linux, then `docker run --rm -v "$PWD":/app -w /app node:22 sh -c "npm ci --production"` to verify — that second command must exit 0. **Symptom on Coolify** if you skip this: Nixpacks build fails at the `npm ci` step with `npm error code EUSAGE` and `Missing: <pkg>@<version> from lock file`. Tests passing locally is *not* a substitute — vitest never runs `npm ci`. `next build` is also not a substitute — it reads `node_modules`, not the lockfile.

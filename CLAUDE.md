...
- **An agent opens the PR and stops; merging to `main` is a human decision.** No `gh pr merge`, no `--auto`, not even on an agent's own green PR. Since 2026-09-12 a push to `main` deploys straight to production ([ADR-0026](docs/adr/0026-production-auto-deploy-from-main.md)), so the merge click is the last human checkpoint in front of a system taking real card payments. Nothing at the GitHub level enforces this (a solo maintainer cannot approve their own PR, and an agent authenticates with the same token), so it is a rule, not a lock. **Never force-merge past a failing check either** — Coolify deploys on the push webhook, not on CI success, so admin bypass reaches production directly.
...
| `/app`, `/app/moje`, `/app/vise`, `/app/dobrodosli`, `/app/login`, `/app/izvedba/[id]`, `/app/statistika`, `/app/authorize`, `/app/set-password`, `/app/forgot`, `/app/instalacija` | `src/app/app/…` (own route group + layout) | Moreškant roster app, Croatian only, noindex, outside `(frontend)`/`(payload)`. Three tabs under `AppShell`/`TabBar` since #457: `/app` the evenings, `/app/moje` one dancer's own season (lineup facts only, never attendance answers), `/app/vise` the once-a-season things. `/app/dobrodosli` is the walkthrough, signed in but chrome-free. Access decision in `src/lib/app/access.ts`; attendance answers write through `POST /api/app/attendance`; a dancer's login is issued by the invitation (`POST /api/app/invite`), never self-registered; `/app/authorize` is the MCP connector's consent screen (#438); `/app/instalacija` is the install guide and the ONLY `/app` page outside the access decision, because it is the QR target for a rehearsal (#455); see `docs/agents/moreskant-app.md` |

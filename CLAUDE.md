...
| `/app`, `/app/login`, `/app/izvedba/[id]` | `src/app/app/…` (own route group + layout) | Moreškant roster app, Croatian only, noindex, outside `(frontend)`/`(payload)`. Access decision in `src/lib/app/access.ts`; attendance answers write through `POST /api/app/attendance`; see `docs/agents/moreskant-app.md` |

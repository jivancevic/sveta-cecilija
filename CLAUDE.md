...
- **Database:** PostgreSQL via `@payloadcms/db-postgres`. Connection via `DATABASE_URL` env var. Auth gated by `PAYLOAD_SECRET`. **Local dev DB is `sveta_cecilija_dev`. Production data lives in the *default* `postgres` database of the Coolify-managed Postgres container (host `tcrw531gbwikko09bvl9ssaz:5432`, superuser `postgres`) — there is no DB literally named `sveta_cecilija`, despite earlier docs; verified on the box 2026-06-02. Staging is a separate `sveta_cecilija_staging` DB (owner `staging_user`) on the same instance.** The three names stay distinct so a misconfigured DATABASE_URL cannot read or write the wrong database. Note: Coolify runs its *own* internal Postgres in a separate `coolify-db` container — never touch it. Local seeds + a throwaway admin account (`admin/admin`) live only on `_dev`; never copy that password to prod.
...
| `/scan/[token]` | `src/app/scan/[token]/page.tsx` (+ `src/app/scan/layout.tsx`) | Auth-aware door scan. Unauthenticated (buyer tapping QR from email) → buyer ticket view with on-page QR + "do not tap again" notice, no DB write. Authenticated internal staff (`isAuthed` — superadmin/admin/tehnika; see `src/lib/access/roles.ts`) → atomic mark-and-read, renders VALID / ALREADY_SCANNED / INVALID. On ALREADY_SCANNED, authed users see an "Undo scan" link for 2 minutes after the original scan (server-enforced, not client-trust). Race-safe. Mobile-optimised. Lives outside `(frontend)` so it has its own minimal root layout (no fonts/CookieConsent). |
| `/admin` | Payload CMS built-in | Admin dashboard — show management, orders, in-person sales, refunds |
| `/admin/stats` | `src/components/payload/AdminStatsView.tsx` | Custom admin view — season aggregate header (tickets sold, scanned, revenue, by-venue split) + show table (upcoming + today + last 7 days). Force-dynamic. Visible to internal staff (superadmin/admin/tehnika). Mobile-friendly. |
| `/admin/stats/[showId]` | `src/components/payload/AdminShowStatsView.tsx` | Per-show drill-down. Role-aware: `tehnika` sees numbers only (online/in-person sold, scanned, remaining, revenue); admin-tier also sees the full order list with buyer PII and per-QR scan state. |
...
|---|---|
| `Shows` | `date` (DateTime), `time` (text, HH:MM validated), `venue` (select: `ljetno-kino` \| `zimsko-kino`), `onlineSold` (number), `inPersonSold` (number), `status` (active \| cancelled), `venueChangedAt` (DateTime, read-only), `venueChangedBy` (→ Users, read-only) — last two are the bad-weather venue-move audit (#94) |
| `marketing_optouts` | **Raw table, NOT a Payload collection** (#57). `email` (PK, lowercased), `source`, `optedOutAt`. Created in `db/schema/app.sql` + `instrumentation.ts`; written by `/api/unsubscribe`, read by the review-email dispatch SQL. See Design decisions. |
...
| `Users` | Payload auth + `role` (select: `superadmin` \| `admin` \| `tehnika` \| `partner`, default `admin`, required — read `src/lib/access/roles.ts`, not this stale enum). **Hybrid username login** (`loginWithUsername: { allowEmailLogin: true, requireEmail: false }`, ADR-0011): every user has a unique `username`; email is optional but **required for `superadmin`/`admin`** via a `beforeValidate` hook (`src/lib/access/user-email-policy.ts`). `partner` logins carry a `partner` relationship → Partners (read-open so it rides on `req.user` for scoping; write locked to admin-tier so a partner can't repoint itself). Shared door account is **username `tehnika`** (no email), created manually via /admin — credentials in Coolify/password manager, never in repo. |
...
- **Bad-weather venue change, Ljetno → Zimsko (#94):** Admin edit-view action `MarkMovedToZimskoMenuItem` (2-step — preview affected-buyer count + sample emails with **no writes**, then confirm). Confirm **claims the move atomically before sending mail**: `UPDATE shows SET venue='zimsko-kino', venue_changed_at=NOW(), venue_changed_by_id=$user WHERE id=$show AND venue='ljetno-kino' AND venue_changed_at IS NULL RETURNING id`. Claim-before-send means two concurrent confirmations can't double-notify (the loser claims 0 rows → reported `already-moved`, no send); also short-circuits `already-moved` / `not-applicable` (natively-Zimsko show). The notice is **deduped by email** (`DISTINCT ON (lower(email))` — one per person, not per order) and is **not a refund trigger** (the show still happens) — buyers who want out reply to `info@` and admin refunds case-by-case via the existing flow. Pure DI orchestration in `src/lib/venue-change.ts`; route `POST/GET /api/shows/[id]/move-to-zimsko`.
- **Marketing-class email + opt-outs (#57):** Only the post-show review email is marketing-class. One-click unsubscribe uses a **stateless HMAC token** (signed with `PAYLOAD_SECRET`, no per-send storage — `src/lib/marketing/unsubscribe-token.ts`) in a footer link **plus** RFC 8058 `List-Unsubscribe` / `List-Unsubscribe-Post` headers (required by Gmail/Yahoo for bulk senders). The opt-out is keyed by **email** in the raw `marketing_optouts` table — by email, not order, so it persists across every future show (each show is a fresh Orders row; a per-order flag would silently re-subscribe on the next purchase). `/api/unsubscribe` writes it (`ON CONFLICT DO NOTHING`, GET = confirmation page, POST = one-click); the dispatch SQL excludes opted-out emails via `NOT EXISTS`. **Transactional mail (ticket confirmation, refund, venue-change) is NOT marketing and never checks the opt-out list** — it concerns a ticket the buyer already holds. A collection-time consent notice (`consent.notice`, EN+HR) sits under the email field on checkout + the `/scan` claim form. All buyer-facing venue names come from the shared `VENUE_LABEL` in `src/lib/venues.ts`.

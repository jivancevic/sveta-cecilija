...
| `/app`, `/app/login`, `/app/izvedba/[id]`, `/app/statistika`, `/app/authorize`, `/app/set-password`, `/app/forgot`, `/app/instalacija`, `/app/povezi` | `src/app/app/…` (own route group + layout) | Moreškant roster app, Croatian only, noindex, outside `(frontend)`/`(payload)`. Access decision in `src/lib/app/access.ts`; attendance answers write through `POST /api/app/attendance`; a dancer's login is issued by the invitation (`POST /api/app/invite`), never self-registered; `/app/authorize` is the MCP connector's consent screen (#438); `/app/instalacija` is the install guide and the ONLY `/app` page outside the access decision, because it is the QR target for a rehearsal (#455); `/app/povezi` is where a voditelj who dances links their own Member, the one sanctioned way past the `Users.member` field lock (#462); see `docs/agents/moreskant-app.md` |

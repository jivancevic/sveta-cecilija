...
export const ALLOW_LIST: Record<string, string> = {
  ...
  'src/lib/app/session-renewal-data.ts':
    'The sliding session’s server reads (#650, #652, ADR-0028 decision 1): `payload.config.cookiePrefix`, so the session cookie is found by the name Payload itself gives it, and `loadDeviceState` beside it, so the six-hour device throttle is decided before the browser is asked anything. Retired with repo.auth (#475) — it is the same “who is asking” family as viewer.ts and session-guard.ts and swaps with them. The RULE it applies imports nothing (session-renewal.ts).',

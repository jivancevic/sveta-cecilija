...
- **Recent orders = one merged list (storno folded in).** The earlier split of a separate "Today's sales" storno panel and a read-only "recent sales" list is collapsed into a **single "Nedavne narudžbe / Recent orders"** list (it showed the same orders). Newest 5, with **"Show more"** paging older orders from a scoped endpoint (`/api/partner/sales`). Each order is a **dense single row**: code · 🕒 sold date+time · 🎭 show date · people · money · a **download-tickets** icon · and (today only) a **cancel** icon. The two dates are disambiguated by **small inline icons** (clock = sold, show-glyph = show; rendered as inline SVG, not emoji, for consistent admin rendering) with tooltips. **Download** (opens the order's always-English ticket PDF) is on **every** row, including older ones (reprints). A sale made **today** (Europe/Zagreb) shows a **chevron** and **expands** to its per-person tickets, each individually cancellable; **earlier-day** orders are flat (no chevron, no cancel — outside the same-day storno window), download-only, under a "RANIJE / Earlier" divider. The cancel control is a **red trash icon**; tapping it swaps the trailing icons for an inline confirm — whole order **"Otkazati narudžbu?"**, per ticket **"Otkazati ulaznicu?"** ([Da]/[Ne]). Discoverability of the per-ticket cancel comes from the chevron **plus** a one-line hint under the heading ("Dodirnite narudžbu za otkazivanje pojedinih ulaznica."). On confirm the order (or its last active ticket) **disappears** from the list (client drop + server `EXISTS active ticket` filter).
- The formal **monthly statement PDF** and the per-show **"Vaše narudžbe / Your orders" season table** are kept below.

...
**Superadmin dev strip.** Collapsed, English/technical (independent of the language toggle), below the shared business dashboard. Diagnostics that no single collection table gives: (1) an **environment + DB guard** banner (prod/staging/dev and which database — protective before mutations, given past prod/staging mix-ups); (2) **data-integrity counts** — per-collection row counts plus anomalies (orders with no tickets, tickets with no order, past shows still `active`, refunds stuck `pending`/`failed`); (3) **integration health** — last Stripe webhook received, last review-email cron run, Stripe balance (available/pending EUR, briefly cached); (4) **quick collection links**; (5) the **critical-events log** (below).
...
**Notification gap (to fix):** since #220 the public forms *persist* to `ContactSubmissions`, but **no email is sent** to the org when one arrives — so an enquiry is only seen by someone who opens `/admin`. A booking lead can sit unseen. The fix: on enquiry create, send a notification email (to `info@`, booking types Cc/route-aware of `bookings@`) so leads reach the inbox, not only the dashboard. The dashboard badge and the inbox notification are complementary, not redundant.

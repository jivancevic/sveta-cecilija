...
A public evening is **two named parts, never three**: the klapa sings, then the Moreška is danced to the wind orchestra. The orchestra also plays a short march before the klapa, and that march is deliberately left unnamed in guest-facing copy. The spoken introduction of the story is named as a clause inside the Moreška half, never as a third act, because it is what reviewers call "lengthy multilingual announcements" and an unnamed introduction misses the point where a guest disengages.

Guest-facing copy writes the runtime as **"about an hour"**, never a precise number, and **never gives a per-part duration**. A precise runtime invites a complaint when an evening runs seventy minutes, and per-part minutes hand the guest a stopwatch, which is the mechanism behind the "it dragged" reviews. Nor does the copy concede ("first there is singing, the dancing comes later"): a guest who knows the combat closes the evening reads the opening as a build-up rather than as a wait.

Croatian says "moreška uz puhaći orkestar"; English keeps "Moreška with live wind orchestra". The copy itself is the top-level **`evening`** namespace in `src/messages/{en,hr}.json` (label, runtime, parts, seating), read through `eveningFacts()` in `src/lib/evening.ts` and rendered by `src/components/EveningBlock.tsx` — a panel on the schedule page, one line on the post-payment receipt, and from #543 the same sentences in the ticket e-mail. It **replaced** `performancesPage.venueDetails` in #542 rather than joining it, because two blocks saying the same thing on one page is the failure mode. Decided in #541 (2026-09-13) against the analysis in `docs/research/tripadvisor-review-themes.md`; #543 and #544 carry it into the ticket e-mail and the digital programme.
...
A screen is one place in Cecilija with one address and one Croatian label (Početna, Moreška, Narudžbe, Izvedbe, Ljestvica, Skener, Prodaja, Obračun, Upiti, Gratis, Korisnici, Statistika, Više). A permission unlocks screens; an account is inside Cecilija when its permissions unlock at least one. The **landing screen** is **Početna** (#564) for every account that is in: it is the first tab, it answers to no permission of its own, and the bar order begins after it. Izvedbe is one screen for everyone who has it: what it shows depends on the permissions, never on a second address. Statistika is the sales screen; dancer statistics live on *Ljestvica*. In code the screens are one table, `src/lib/app/screens.ts` (#495): a screen the app has not built yet is in that table with an empty `servesToday`, so it is named and ranked without being offered.
...
Each line carries a **ticket type** (`adult | child`, the same two the rest of the system uses), a **quantity**, the **unit price actually charged**, and an optional **discount label**. A discounted seat keeps its real type: 32 pensioners at €15 are *adult* lines with a discount label, **not** a third price category. Prices are stored on the line so a later price change cannot rewrite history.

The label answers **"why does this line exist"**, not only "why is it cheaper". It is *required* below face value, because a price nobody explained is a price nobody can audit; it is *allowed* at face value, and that is not an abuse of the field. A block of seats sold in advance to one buyer is the case that shows why: 80 children of one school group at the full €10 child price is an ordinary ledger line in every arithmetic sense, and without the label the ledger says only that somebody sold 80 child seats at the door a month before the evening. The name of the column is historical; the question it answers is the wider one.
...
- **Revenue collected** — money actually in hand: online orders (Stripe, net of refunds) + the **offline sales ledger** (door and legacy lines, summed as `quantity × unit_price_cents`, so a child seat and a discounted seat are valued at what was actually charged). It is never a headcount multiplied by a flat price. The bare `totalRevenueCents` is *online gross only* and must not be presented as the whole. **"Online" is a channel clause, not a manner of speaking** (#538): the one implementation, `revenueCollectedCents` in `src/lib/dashboard/revenue.ts`, drops every order whose `channel` is not `online`, because a partner order stores `total` at face value the society has not collected and a **storno** voids only the tickets, leaving `total` and `refund_status` standing for ever. `CollectedOrderRow.channel` is required, so a surface that forgets to read the column fails `tsc` rather than quietly inflating the figure.
...
The app produces a **per-partner monthly reconciliation statement** — NOT a fiscal invoice. The legal `račun` is issued by the secretary in their own accounting/fiscal tool (avoids fiskalizacija liability **for this partner/B2B channel only**). **The online B2C channel is separate: direct card sales to consumers via Stripe DO require a fiscalized račun (ZKI/JIR) — HGD is a confirmed obveznik fiskalizacije (obveznik poreza na dobit). This is a known gap, not yet implemented; tracked in #297.** The statement, viewable/exportable by admin and the partner, breaks down for the calendar month: tickets sold by show and type, cancelled (storno) count, **gross** (`sold × face value`), **commission** (gross × partner `commissionPercent`, Kaleta 10%), and **net payable**. Period is bucketed by **sold date** (`order.created_at`); because storno is same-day-only, the month-end active-ticket count is already final. The document **says so out loud** since #599 (*Razdoblje: prodaja 1. 8. do 31. 8. 2026.*), because the partner charged the guest at their counter and closes their till by sale day: that is the only date on which the two tills can be compared, and it is not the evening the guest attends. The **season receivable is the same query with a year window**, so it is the sum of the twelve statements by construction rather than a second figure that resembles them.

**Vrijednost prodanog** is the one name for the gross (#599): the face value of everything the partner sold in the period, before commission. It had three names — *Ukupno naplaćeno* on the partner's Obračun, *Naplaćeno* on Financije, and both sat one line from *Prikupljeni prihod*, which means something else entirely (money the society actually holds). Partner money is not collected revenue and the two are never summed (see *Dashboard*). The other two words are **Provizija** and **Za uplatu HGD-u**, and those three are what the screen, the PDF and the spreadsheet all print.

**Sending it freezes it** (#599). Marking a month sent stores the whole document — the partner's name, OIB and address as they stood, every izvedba line and every cent — and every later read of that month serves the stored one. Before that, commission lived only on the Partner and was applied at read time, so raising a reseller's rate silently rewrote months the accountant had already invoiced. A month that has not ended cannot be marked sent; un-marking returns it to live and is how a mistake is fixed. Payment itself is **not tracked**: that is the accountant's book, and the app would be guessing.

**The document carries no IBAN and no payment instruction.** The partner pays against the *račun* the accountant issues; a bank line on the statement would invite a transfer outside that invoice and leave the accountant unable to match it. One closing sentence says so instead: *Na temelju ovog obračuna knjigovodstvo izdaje račun.*
...
### Financije
The money screen of Cecilija, live at `/app/finance?season=` since #509 (decided in #476). Unlocked by `finance`, never by `tickets` alone, and it shows no buyer: *Prikupljeni prihod* for the season (`channel='online'` order totals net of the refunded ones, plus the offline ledger's own Σ quantity × unit price), *Povrati* as a count and an amount, *Promo kodovi* as an "of which" line, the per-partner receivable for one month with the statement CSV behind it, and the season's door-sales ledger evening by evening, each line at the price it was charged and with the label saying why it was below face value.

**Collected means collected, which is why only the online channel is in it.** A partner order stores `total` at face value the moment the reseller issues the seat, but the society sees none of it until the monthly obračun, so it belongs to the receivable and to nothing else; and a *storno* voids the tickets while leaving `total` and `refund_status` untouched, so a cancelled partner sale would otherwise sit in revenue for ever. A comp order is €0 (ADR-0019) and drops out of the same clause rather than needing a second rule. A DEACTIVATED partner still owes for what it sold, so the receivable is derived from the sales rather than from the list of partners who may still sell, and the month panel badges such a partner *neaktivan* instead of hiding it.

**Prikupljeni prihod and Potraživanje od partnera are two facts and the screen keeps them in two cards**, in that order, with a sentence under each saying what it is not. They are never summed and the word *dobit* appears nowhere: the society records no costs, so a bottom line would be a mislabelled gross. That is the same rule the *Dashboard* entry below states for the Backoffice band this screen replaces, and the layout is built around it — there is deliberately no laptop rule that puts the cards in a row, because a band of euro tiles side by side is what invites the addition.

*Statistika* is its counterpart: counts and fill for `tickets`, `season_stats` and `finance`, never money. The two screens never print the same figure.

### Korisnici
The account screen of Cecilija, live at `/app/users` since #510, unlocked by `users` and nothing else. It lists every login with its permission set written out in Croatian, the *shared* badge, the address (or "bez e-pošte", which is what decides how a password is handed over) and both links by name, and it changes an account only through named actions: Dozvole, Resetiraj lozinku, Poveži partnera, Poveži člana, Dijeljeni račun, plus Novi korisnik. One person is one account (#487): a second job is a permission added here, never a second login. Deleting an account is not on this screen and stays Backoffice work; a login is retired by taking every permission off it, which leaves the history it wrote intact.

### Predaja pristupa (handover)
How a person first gets into an account somebody else opened for them, and there are exactly two ways. An account whose e-mail is on file AND whose permissions unlock a Cecilija screen gets a **sign-in link**, the same one-hour `/app/session?token=` link "Zaboravljena lozinka" mints, which the `users` holder forwards. Every other account — the shared *tehnika* door login, a partner POS, an `editor` whose whole job is in the Backoffice — gets a **privremena lozinka**: fourteen characters drawn from an alphabet with no `l`, `I`, `1`, `O` or `0`, shown on screen exactly once, never e-mailed and never logged, with only Payload's hash of it kept. The link is not single-use, and an account holds only one live token, so issuing a second link kills the first without telling anybody. A *Pozivnica* is the dancer's version of the same idea and is issued by a voditelj, not here.
_Avoid_: "temporary account", "password reset e-mail" (Cecilija sends none from this screen).
...
### Zaduženi (za popis)
(#658) A moreškant a voditelj has put in charge of **one evening's list**: they answer for the dancers who did not, move somebody between the armies, hand out the four *titles*, edit the *Lineup (postava)* and lock or unlock it — on that performance and no other. It is the answer to an evening where neither Josip nor Branimir is present, and it is not a rank: the same person has it on Tuesday and not on Wednesday. Several may hold it at once, and whoever holds `moreska` always does, on every evening, without being named. It does **not** carry the *Alarm*: ringing the roster stays the voditelj's, and so do the two army thresholds that travel in the same sheet. It has no expiry and survives the evening, because a postava is confirmed after the night it records. Held only by a moreškant with a login; a dancer who goes inactive or loses their login simply stops holding it, and the record on the evening is left alone. Given and taken back on *Stanje*, by a `moreska` holder only — a zaduženi cannot pass it on. *Stanje* names one out loud ("Popis vodi: …") only when it is somebody **other** than a `moreska` holder; when the line is absent, the voditelji are keeping the list, as always.
_Avoid_: voditelj (both senses are taken), delegate, moderator, "privremeni voditelj" — it is not a smaller voditelj, it is a different thing with a smaller scope.
...
### Koncert
The society singing and playing: the wind orchestra, the klapa, the choir. A `shows` row like any other, and **not a moreška** (#635) — counting one as one inflated a dancer's season and broke a *Niz* on an evening nobody danced. Since #635 Cecilija does not show one **at all**: it is in no list, no count, no chain, no picker and no calendar feed, and its detail page is a 404. The kind stays in the `shows` enum and in the Backoffice, because the musicians will get a screen of their own and it comes back with them.
_Avoid_: nastup, izvedba (both are the dancers' registers, and a koncert is in neither).

### Vanredna izvedba
Any performance that is neither a *redovna*, a *Moreška Experience* nor a *Koncert*: a ship group (DMC, Gulliver), a festival, a charity evening. The word the notebook uses for such an evening ("vandredni nastup 28.07. 19:00"). Not a kind of its own: it is the complement of `redovna`, `experience` AND `koncert` (so `dmc`, `gulliver`, `ostalo`), and the specific kind still says what it was. The Experience used to read as a vanredna too; since the app shows three categories it reads as itself.
...
**Un-tapping your own answer goes through the same ten minutes** (#624). The season list carries two circles per future nastup and the filled one can be pressed again to take the answer back; what that does is decided on the server, not by the shape of the gesture. Inside the window the row is deleted and the dancer is back to having said nothing; past it the row becomes *ne dolazim* carrying the odustajanje, and the red circle fills. Un-tapping the RED circle deletes an ordinary *ne dolazim* and leaves an odustajanje exactly where it is: an odustajanje is not un-said by pressing a button again, and the one way back out of it is *Dolazim*, which clears the trace because coming back is what the trace was waiting for — a place somebody was counting on has emptied, and saying so is the whole job of this list. It is therefore **not** the voditelj's *Poništi* on the person sheet, which deletes the row unconditionally: that one corrects the RECORD ("this answer was never given"), while this one is an act by the person, and an act leaves a trace.
...
**A dancer answers from three places and it is one answer** (#624): the hero on *Moreška*, the two circles on every future row of the season list under it, and the pair at the top of *Stanje*. The shapes differ because the space does — a full-width button where there is one evening, a 34px circle where there are fifteen — and the rule behind all three is the single `POST /api/app/attendance`. Only the circles take a second press as a take-back; on a full-width button a double tap is a fat finger, not a decision.
...
The only way a moreškant gets a login. A voditelj presses "Pošalji pozivnicu" on the Member; the account is created with exactly the `moreskant` permission and the Member link, and the dancer gets a Croatian email with a link to choose a password, **valid for seven days**. Pressing again sends a fresh link to the same login, never a second one. An invitation may land only on a login whose permissions stay inside `moreskant` and `door` (#520); aimed at any other staff account it refuses, because a press mints a live sign-in token for whatever login stands behind the Member. A dancer who forgets their password asks for one themselves from the login page and gets the same page behind a **one-hour** link. Since #463 the usual channel is a LINK rather than a letter: "Kopiraj pozivnicu" mints a seven-day sign-in link a voditelj hands over by SMS, because most of the roster has no e-mail address. Since #511 both channels live on the dancer's own row on *Članovi* rather than on a screen of their own.
Since [ADR-0028](docs/adr/0028-moreskant-owns-their-access.md) it is no longer the only way back in: a dancer is asked to set an **e-mail and a password together**, once, and from then on recovers their own access. The invitation stays a first-class channel and is never taken away, because roughly half the roster will never type an address.
_Avoid_: registration, sign-up, account request.


### Pristup (a dancer's own access)
What a moreškant needs to get back into Cecilija without asking anybody: an **e-mail** and a **password**, set together as one task ([ADR-0028](docs/adr/0028-moreskant-owns-their-access.md)). The e-mail lives on the login (`Users.email`), written by the dancer on their own *Profil*; `Members.email` is retired and with it the rule that the Member's address wins. The **mobile is the voditelj's channel and the e-mail is the dancer's access** — one sentence that decides which of the two anybody may edit. A dancer who sets neither is not broken and is never locked out: their way in stays a voditelj-minted *Pozivnica*, permanently. The **session slides**: any request under `/app` on a cookie older than seven days renews it for thirty, so only a month away signs anybody out.
_Avoid_: registracija, "aktivacija računa", onboarding (that is *Dobrodošlica*).

### Uređaj (device)
One browser a moreškant has opened Cecilija in, recognised by a long-lived server-written cookie rather than by its user agent, so two iPhones are two devices. What the app remembers about one is only what the roster needs to read: whether it is running as an **installed app** and whether it can be rung. *Instalirano* and *obavijesti* are therefore facts about a person **across all their devices** — either counts — and both have a third state, **ne znam**, for a device that has not reported yet. A dancer's phone and tablet both ring; *Znak* and every dancer-facing screen know nothing about devices.
_Avoid_: session (a session is a login that expires), endpoint (that is the push service's word for one subscription).

### Članovi
The voditelj's roster screen in Cecilija at `/app/members` (decided in #476, built in #511): the list of moreškanti with a search, each dancer's profile (nadimak, mobile, e-mail, dance roles, primary role, active), adding a dancer by hand, and every *Pozivnica* affordance (the rehearsal join code, pending claims, invitations per dancer). Active moreškanti first, retired ones under them rather than hidden. Unlocked by `moreska`; a moreškant does not open it, because reading the roster is a dancer's and editing somebody's profile is not. It absorbed the former Pozivnice screen, so `/app/invitations` and `/app/pozivnice` both 308 here. Deleting a member, the attribution half of a *Member* (`ime`, `bilješka`) and promo codes stay in the Backoffice. Every row carries **three marks** on its right, one glyph per column so the list scans down rather than across: installed, notifications, *Pristup*. Full colour means yes, dimmed means no, a dash means **ne znam** — which is what the whole install column says on day one, and it must not read as "nobody has it". A dancer who has **never signed in** gets none of the three, but the words **nije ušao**: what to do about it is on their profile, where the two invitation buttons already live. Above the list, chips carrying their own counts — **svi · bez appa · bez obavijesti · bez pristupa** — because "who do I still have to ring" is a question about a set, not about a row.
_Avoid_: roster as the screen's name (roster is the whole membership), "članovi" for the comp-attribution list, which is the *Member* half.
...
### Lineup (postava)
Who danced which dance role at a performance: one entry per member, exactly one role each. Enterable before or after the performance; **confirmed** by a voditelj marks it final. In practice confirmation happens **after** the evening, not before it: a postava is a record of who actually danced, so what a voditelj confirms is the night that happened, and the screens read that way (a past row carries *POPIS*, an upcoming one carries the reader's own answer). Only confirmed lineups feed statistics.

A postava therefore has two states, and **confirmation decides what COUNTS, never who may LOOK** (#670). A **nacrt** is a postava somebody is still assembling; a **potvrđena** one is the record of the evening and the only kind that reaches the statistics and *Ljestvica*. Both are readable by every moreškant, past and future alike — a dancer coming to the plac may know the podjela, and until #670 they could not, because confirmation comes after the night and so the crowns appeared a day late. What stays with whoever keeps the list is not the postava but the **instruments** for assembling it: the picker, the suggestion built from the answers, the role warnings and the tally of titles given. A dancer can act on none of those, and "nedostaje bula" reads as breakage to somebody who cannot fix it. Editing is unchanged: a voditelj or that evening's *Zaduženi*, and only while the postava is a nacrt. A role outside the member's profile is a warning, not a block. The MCP `set_lineup` tool always writes an *unconfirmed* lineup. A lineup may also carry one line that is not a dance role: the *Voditelj (u postavi)*. It is written on *Stanje*, the one screen that edits it, by a voditelj or by that evening's *Zaduženi*.
_Avoid_: cast, roster (roster = the whole membership).

### Title (titula)
One of the four named parts of a performance: **crni kralj, bili kralj, otmanović, bula**. A title belongs to one performance's lineup, never to a person: it is given by the voditelj for that nastup, and a confirmed lineup carries **exactly one of each**, no more and no fewer (decided 2026-09-13). Until the voditelj gives them, every moreškant who said "coming" is a plain crni, bili or bula in the *Stanje*, whatever their profile's *Primary role* says; the profile role decides only which army the answer counts in. Once given they are worn **immediately**, on the nacrt, without waiting for the confirmation that comes after the evening (#670). A title is what the dancer wears in the app that evening (the crown on the mark); the profile keeps the *Dance roles* a person is able to dance. **The bula is the one title that cannot be told apart from its army**, because the *Lineup* vocabulary has no plain `bula` role under it: a bula who answers "coming" is the bula of that nastup, and when two of them come only one is in the postava (the other stands in the Bule card without a line in it). Taking the bula off somebody therefore takes them out of the postava, while taking a crown off a king leaves him a plain crni or bili.
_Avoid_: rank, position; "kralj" as a permanent attribute of a person.

### Znak (the mark)
The disc beside a moreškant's name. It means **one of two things and never both**, and which one depends on where it is drawn:

- In a **profile** context — a person shown as themselves (Moreška's header, *Više*) — the disc is the army of their *Primary role* and the glyph is that role. It says what this person dances.
- In a **nastup** context — a person shown inside an evening (*Stanje*, the postava, the hero) — the disc is the army their answer counts in and the glyph is that evening's *Title*, or nothing. It says who they are tonight.
...
The dancer's own tab in Cecilija (decided in [Cecilija: route map](https://github.com/jivancevic/sveta-cecilija/issues/473)): a moreškant's own season on one side and the season's moreškanti ranked by confirmed performances on the other, the same count as *Dancer statistics*, shown in full with nicknames. The season is **two lists** ranked apart, Moreška and Experience. The Moreška list is every kind Cecilija shows except the Experience, which since #635 means the *Koncert* is in neither. Every active moreškant is on it, equal counts share a rank and the next one skips. The count is a lineup fact, never an attendance answer, so saying "coming" moves nobody.

The top three of a list wear a **cup** carrying their rank, so a tie reads as two golds and no silver. Every disc is the dancer's army with their **initials** in it and never a crown: a *Title* belongs to one evening's lineup, so nothing in a season's counts has one to draw. The reader's own row also carries how far they **moved** on the last confirmed evening, up or down, and nobody else's does.
...
Every row, and every step of a podium, opens that dancer's **season profile** (see below). **No milestones and no "puna sezona"** since #607: 5, 10, 15 and 20 were a ladder the app invented, and both were deleted rather than replaced. Since #628 a row may carry a **flame with a *Niz*** beside the nickname, and that is the one exception to "no streaks": it is not a season's number, nothing is ranked by it, and it is drawn only while the run is still going. Past seasons selectable.
_Avoid_: points, prekretnica, ranking by answers, "Moje", statistika (that word is the sales screen).

### Niz
The unbroken run of confirmed *Lineups* a moreškant is in, counted backwards from the most recent moreška danced. It **crosses seasons**: it does not reset in January, or every niz would die at the end of summer.

Four rules decide what is a link in the chain and what is not. Only a **confirmed** postava counts, because only a confirmed postava says who danced. A **Moreška Experience neither counts nor breaks** one: it is not a moreška, and the two lists are already ranked apart. **Neither does a *Koncert*** (#635), and that one had teeth: the society singing and playing is not a sword dance, so until it left the chain a dancer who skipped a concert lost a run of evenings he had actually danced. A **cancelled** evening is skipped, not a break, because nobody could have danced it. A past evening that was **never confirmed** is skipped too, and that one is deliberate: its absence is an unfinished record rather than somebody's fault.

Drawn as a **flame in ember orange**, the one colour in Cecilija used for nothing else, and only when it is **3 or more AND still running** — a run that has been broken is not drawn at all, whatever its length. The threshold and that second half were both settled against the real season (#628): ten dancers had a run of three or more and only four were still in one, so six of ten flames would have been a record of somebody who stopped dancing in May. In a list row the number sits beside the glyph; on a *Season profile*, where there is room, it sits inside it.

**Najduži niz** is the record: the longest run ever, across all seasons, which took "Najbolja sezona"'s place on the profile. A best season can only be beaten once a year and is settled by August; a longest niz is a record somebody can break next Wednesday.
_Avoid_: streak, serija, niz nastupa (an Experience is a nastup and is not in the chain).

### Season profile (profil moreškanta)
One moreškant's year on one screen, at `/app/leaderboard/[memberId]`, behind the same permission as *Ljestvica* (#608). Their name and nickname, the roles they are able to dance, a ring for each of the two lists with its count, place and role breakdown, their longest *Niz* ever, the evenings they danced with the role worn in each, and the season month by month. **Moja sezona is the same screen with the reader in it**, and the only difference is one block: the standing sentence and the rank movement, which are a message to the reader and belong on nobody else's profile.
...
### Notification types
Push is the only channel that reaches a phone; since the route map decision every notification is also kept in the *Sandučić obavijesti* for reading later. (1) Alarm; (2) answer reminder at T-48h to *no answer*, and its window **closes 24 hours later** — a performance entered inside T-24h gets no reminder at all, the alarm covers it; (3) performance change (date, time, place, cancellation, voditelj note) to everyone except *not coming*, cannot be muted; (4) new performance to everyone. A fifth kind, "a coming withdrawn within 24h" to the voditelji, was **retired** in #612: a dancer changing their mind is written under the evening, in *Odustali* on Stanje, and rings nobody. Two further kinds are filed and **never** pushed, both to `tickets` holders (#496): a new inquiry and a new card dispute. Never an order. A **seventh** kind arrived with [ADR-0028](docs/adr/0028-moreskant-owns-their-access.md) and is the first that is not event-driven: a **voditelj's message**, free text to every active moreškant, written from *Obavijesti* (the action is hidden, not greyed, for a reader without `moreska`, because nobody needs to ask a voditelj to write to the dancers). It is filed in the *Sandučić* for all of them and pushed to the phones that can be reached, and sending is confirmed on a sheet that says both numbers out loud, because it rings dozens of phones and cannot be taken back. In code the seven are `AppNotificationKind` (`src/lib/app/notification-audience.ts`), and a push message carries its kind so the push and the inbox row are one write. A user may hold several push subscriptions, **one per device**: the endpoint is unique across the whole table, not per user, so a shared phone rings for whoever signed in last rather than for both.

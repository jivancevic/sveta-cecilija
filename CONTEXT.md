...
How a person first gets into an account somebody else opened for them, and there are exactly two ways. An account with an e-mail gets a **sign-in link**, the same one-hour `/app/session?token=` link "Zaboravljena lozinka" mints, which the `users` holder forwards. An account without one — the shared *tehnika* door login, a partner POS — gets a **privremena lozinka**: fourteen characters drawn from an alphabet with no `l`, `I`, `1`, `O` or `0`, shown on screen exactly once, never e-mailed and never logged, with only Payload's hash of it kept. A *Pozivnica* is the dancer's version of the same idea and is issued by a voditelj, not here.

...
**Soft opt-in implementation (#148, shipped):** the review email carries a **working unsubscribe link** and both collection points show a **consent notice**. Mechanism (`src/lib/review-consent/review-consent.ts`): two columns on `orders` — `review_opt_out` (boolean) + `review_opt_out_token` (unique, generated lazily when the review email is sent). The cron route ensures a token and embeds `${BASE_URL}/unsubscribe/[token]` in the email footer (EN + HR). The public page `/(frontend)/unsubscribe/[token]` resolves the token via an atomic `UPDATE orders SET review_opt_out = true WHERE review_opt_out_token = $1` (idempotent — a re-click re-matches; an unknown token shows a neutral "not recognised" page). The dispatcher's eligibility SQL adds `AND o.review_opt_out IS NOT TRUE`, so opted-out orders are skipped. A one-line collection notice (`consent.notice` in the dictionaries; hardcoded EN on the minimal `/scan` claim form) sits under the email field on **both** checkout and the claim form. Behaviour is identical for online and claimed-partner buyers (the dispatcher operates on orders regardless of channel).

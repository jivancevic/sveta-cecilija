...
### Članovi
The voditelj's roster screen in Cecilija at `/app/members` (decided in #476, built in #511): the list of moreškanti with a search, each dancer's profile (nadimak, mobile, e-mail, dance roles, primary role, active), adding a dancer by hand, and every *Pozivnica* affordance (the rehearsal join code, pending claims, invitations per dancer). Active moreškanti first, retired ones under them rather than hidden. Unlocked by `moreska`; a moreškant does not open it, because reading the roster is a dancer's and editing somebody's profile is not. It absorbed the former Pozivnice screen, so `/app/invitations` and `/app/pozivnice` both 308 here. Deleting a member, the attribution half of a *Member* (`ime`, `bilješka`) and promo codes stay in the Backoffice.

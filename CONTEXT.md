...
- **Post-sale = non-blocking confirmation, form stays.** Issuing a sale opens the combined PDF in a new tab and shows an **inline success banner** (issued count + order code) with a **5-second countdown bar**, after which it auto-dismisses. The sell form is **not** replaced by a "Sale complete" card; it stays put with counts reset to 0, ready for the next sale, and the dashboard's own figures (recent sales, this-month, season) **refresh in place** (`router.refresh()`) — no full page reload, no stale "nothing happened until I refresh the whole site".
...
- **Recent sales = one merged list (storno folded in).** The earlier split of a separate "Today's sales" storno panel and a read-only "recent sales" list is collapsed into a **single Recent sales** list, since they showed the same orders. It shows the **newest 5**, with **"Show more"** loading older pages on demand from a scoped, paginated endpoint (`/api/partner/sales`). A sale made **today** (Europe/Zagreb) is **expandable** to its per-person tickets, each with a **Cancel** action, plus a **Cancel whole sale** button; **earlier-day** sales are **flat read-only** rows (no expand, no cancel — outside the same-day storno window). Every cancel action carries an **inline "Sure? Yes / No" confirm** before voiding (destructive: frees a seat and drops the ticket from the monthly invoice). Heading note: "You can cancel a ticket or a whole sale on the same day it was made."
- The formal **monthly statement PDF** and the per-show **"your sales" season table** are kept below.
- **Croatian by default, and actually localized.** Every partner-facing string (sell form, stepper, success banner, recent sales, season table, statement, errors) runs through `adminT(lang, …)` with HR + EN copy — not just the month-to-date card. Partners default to HR (`defaultLanguageForRole`), so an un-localized literal silently showed English; the whole surface must track the active admin language.

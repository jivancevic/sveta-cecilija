...
5. **Google Ads side-finding (verified 2026-07-14 in the Ads UI):** conversion tracking **is** set up — a GA4-imported action "moreska.eu (web) purchase" exists (source: Website / Google Analytics GA4, status Active, Primary, in the account-default Purchases goal, attached to the campaign). The "Set up conversion tracking" banner is Google's generic nudge because the account has 0 recorded conversions ever — expected, since the €2/day TEST campaign only started serving ~13–14 July (€3.97 spent in 7 days, a handful of clicks). Tracking status "No recent conversions" = link works, nothing to import yet. Residual check: the ad-click → GA4 purchase → Ads import chain is unproven end-to-end until the first ad-attributed purchase; only if Ads still shows 0 after meaningful click volume while GA4 records `google / cpc` purchases is there a real problem.
...
- [ ] **Ads owner:** no action needed on the "Set up conversion tracking" banner (GA4 purchase import verified configured); re-check Ads conversion counts once the campaign has meaningful click volume.

...
**1. A user holds a set of permissions; the role enum is removed.** `Users.permissions` is a multi-select over `users | tickets | refunds | door | partner | season_stats | moreska | moreskant | dev` (amended by [#500](https://github.com/jivancevic/sveta-cecilija/issues/500), which added `finance` and `editor`; the live list is always `src/lib/access/permissions.ts`). All checks go through `can(user, permission)` in `src/lib/access/permissions.ts` (the module landed under its own name in #394 rather than inside `roles.ts`, which #397 deleted); no code compares a role string. `partner` requires a Partner link, `moreskant` requires a Member link (`Users.member`). "Superadmin" survives only as shorthand for "every permission". The old roles map to fixed permission bundles at migration time (admin → tickets+refunds+door; tehnika → door; partner → partner; member → season_stats; superadmin → all).
...
**3. `/app` is the primary staff surface; Payload admin becomes backoffice.** *(Superseded 2026-09-12 by [ADR-0027](./0027-cecilija-one-staff-app-payload-backoffice.md): the surface is named **Cecilija**, it is the only daily surface rather than the primary one, the raw-edit surface is the **Backoffice** behind `dev`, and the access decision now reads "in when the permission set unlocks at least one screen" instead of `voditelj | moreskant | denied`.)* A new mobile-first route group, brand CSS, sharing the Payload auth cookie, hosts the roster app and, screen by screen as they are touched, the existing custom dashboards (stats, partner sell, comp issue, bulk create). Payload keeps auth, the local API, and stock CRUD for rare raw edits. Nobody is expected to use `/admin` daily.

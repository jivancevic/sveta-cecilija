...
**Status:** **Accepted** — MX cutover completed 2026-08-13 (#223 closed). Mail for `moreska.eu` is delivered by Google Workspace and `info@` is a real mailbox. The free-nonprofit activation was **not** how this landed: it was rejected twice, so a **paid Business Standard seat** was bought instead (one Flexible seat, on `info@`), with `admin@` and a break-glass `rezerva@` on free Cloud Identity licences. Outcome differs from Considered option 5 only in edition, not in shape. **The driving requirement is now met: the secretary has direct access to `info@` as of 2026-08-21 (#373 closed), so org mail no longer sits in a personal inbox and no longer depends on Josip to relay it.** The free-nonprofit downgrade is still being pursued as a cost fix, not a capability fix (#370): the org-to-domain objection that caused three rejections was cleared on 2026-09-11, once the registered street address was published in the site footer and the `Organization` JSON-LD (PR #446), and the Google for Nonprofits console now reads "Eligible for Google Workspace discounts" for `moreska.eu`. The paid seat still bills €16.20/mo until Google moves the subscription, which they must do for us because Business Starter is not offered in this tenant's catalog.
**Date:** 2026-06-02
**Supersedes:** ADR-0004

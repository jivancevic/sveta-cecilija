...
- **`tehnika`**: door-staff tier. Renamed from `door-staff` to match the shared login already in use (now **username `tehnika`** — the `tehnika@moreska.eu` email string was retired by [ADR-0011](./0011-hybrid-username-login.md); originally [ADR-0004](./0004-email-infrastructure.md)). Behaviour is identical to the previous `door-staff` role: read-only stats access, authenticated `/scan/[token]`, undo-scan-within-2-minutes.

...
- `MCP_CLIENT_ID`, `MCP_REDIRECT_URIS` — the Claude connector's OAuth client (#438). `MCP_CLIENT_ID` is any stable string the client sends back (`claude` in prod); `MCP_REDIRECT_URIS` is a **comma-separated allow-list matched exactly**, and in production it is `https://claude.ai/api/mcp/auth_callback`. There is deliberately **no client secret**: the connector is a public client, PKCE S256 is the proof, and a shared constant pretending to be a secret would be worse. Neither is a secret in the "rotate on leak" sense, but both must be set or `/app/authorize` and `/api/oauth/token` refuse the flow. Changing `MCP_REDIRECT_URIS` does not invalidate any issued token; changing `MCP_CLIENT_ID` stops the next authorization, not the existing connections. **Revoking a connector** is `DELETE FROM oauth_tokens WHERE user_id = …`, or simply removing that account's `moreska` permission in `/admin` (re-checked on every call).

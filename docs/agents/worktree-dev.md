...
`.github/workflows/ci.yml` triggers only on `pull_request` to `main` (and push to `main`). A PR opened with a base branch other than `main` — a *stacked* PR — gets **zero CI runs**, so the five required checks (`vitest`, `lint-types`, `build`, `docker`, `drift`) never report and the PR sits `BLOCKED` ("waiting for status") even though it's `MERGEABLE`.
...
> **An agent may run the `gh pr merge` calls above once every check is green** (CLAUDE.md, granted 2026-09-12 — it replaced the older rule that reserved the merge click for a human). A pending check is not a passing one: read all five before merging. `gh pr merge --admin` will no longer get you past a red check — since #649 `enforce_admins` is **true** ([ADR-0028](../adr/0028-ci-is-the-last-gate-and-the-bypass-is-closed.md)), because a push to `main` deploys straight to production ([ADR-0026](../adr/0026-production-auto-deploy-from-main.md)) and the gate only costs about three minutes. There is no emergency path. If a required check is wedged rather than failing (a stacked PR, see above), fix the PR's base — do not reach for the flag.

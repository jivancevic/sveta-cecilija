...
| partneri (nominalno) | the adult/child split of the evening's ACTIVE partner tickets, priced at the fixed €20/€10 by `partnerFaceValueCents` |
...
`partnerReceivableNote()` answers `null` on the same rule, so the one line
...
under Prihod reads *"Partneri: 12 ulaznica, nominalno 220,00 € (potraživanje)"*,
omitted when the evening sold none, and never summed into anything.

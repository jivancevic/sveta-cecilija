...
`Shows` is the record of **every** performance, not only the ticketed ones. `kind` (`redovna | dmc | gulliver | koncert | experience | ostalo`) says what it is; `isPublic` says whether it sells. A **public** performance is what a show has always been: venue, capacity, online and partner sales, `/tickets`, door scan, ticket statistics. A **non-public** one (a cruise-ship call, a concert, a one-off) carries a free-text `location` and an optional `client` (the ship or organiser), `venue` NULL, no capacity, no sales, and is hidden from every buyer, partner, door and ticket-statistics surface. Save-time rules (`src/lib/show-performance.ts`): `redovna` must be public, public needs a venue, non-public needs a location and gets `venue` NULL plus `inPersonSold` / `legacyReserved` zeroed and `onlineSalesPaused` false.

**A kind can be HIDDEN from Cecilija without leaving the enum** (#635). `HIDDEN_KINDS` in the same module is the one list, `SHOWN_KINDS` / `isShownKind` / `SHOWN_PERFORMANCE_WHERE` the three shapes every caller reads it in, and `MORESKA_KINDS` (the shown kinds minus the Experience) is what *a moreška* means to the Ljestvica list, the profile's rings and the *niz* chain. Today it holds one value: `koncert`. The society singing and playing is not a sword dance, and counting one inflated a dancer's season and broke a run on an evening nobody danced. So no list, count, chain, picker or calendar feed carries one and the detail loader refuses one by id (a 404 on Stanje and on the Izvedbe detail), while the value stays in the `shows` enum and in the Backoffice for the musicians' own screen. `lineupRequirements('koncert')` is deliberately left alone: unreachable rather than wrong, and repurposed rather than rebuilt when they arrive.

There is exactly **one** spelling of "is public" — `PUBLIC_PERFORMANCE_WHERE`, `publicPerformanceSql()`, `isPublicPerformance()` in `src/lib/show-performance.ts` — and `src/lib/show-performance-guard.test.ts` scans `src/` and fails the build on any shows read that neither uses it nor is on the allow-list with a justification. Who may edit what: `tickets` owns public rows (dates, venue, status, sales); a `moreska` holder (the voditelj) creates, edits and deletes non-public rows and may set the roster fields (`thresholdCrni` / `thresholdBili`, default 8/8, and `voditeljNote`) on any row, but can never flip a row public or touch a public row's sales fields; `door` sees public rows only. Since #503 the voditelj does all of that from Cecilija rather than the Backoffice — the four routes and their refusals are in `docs/agents/moreskant-app.md` → *The voditelj's own izvedbe*.

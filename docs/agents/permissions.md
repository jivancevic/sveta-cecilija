...
| `moreskant` | A moreškant's own roster view at `/app` (ADR-0024). Since #421 it is the dancer half of the `/app` access decision; on Members and Users it grants nothing, so a `moreskant` login reaches no collection but its own row. |

...
- **A Member is one row, two audiences.** Like Shows, `Members` is shared by the backoffice and the voditelj since #420, and the predicates live in `src/lib/access/members-access.ts`, not in the collection. Read, create and update are `tickets` **or** `moreska`; delete stays `tickets` (a Member row carries comp history, so a voditelj retires a dancer with `active`, never by deleting). The split is field-level: the six moreškant fields (`isMoreskant`, `nickname`, `mobile`, `email`, `roles`, `primaryRole`) lock **read and update** to `moreska`, so a `tickets`-only form is the ADR-0019 attribution form it always was; `name`, `active` and `note` stay writable by `tickets` only. The sidebar is hidden from anyone holding neither.
- **`Users.member` is the dancer link and is locked to `users` for read and update**, unlike `Users.partner` (whose read stays open). Nothing on `req.user` needs it: `/app` re-reads the account with `overrideAccess` in `src/lib/app/viewer.ts`, precisely so a moreškant cannot repoint themselves at someone else's roster identity.
...
- **Attendance is the voditelj's table, scoped to a dancer's own rows** (#422, `src/lib/access/attendance-access.ts`). `moreska` reads and writes everything; a `moreskant` gets `{ member: { equals: <own> } }`; nobody else reaches the collection, the ticketing backoffice included, and the sidebar entry is `moreska`-only. Two gotchas: the access functions are **async**, because `Users.member` is field-locked to `users` and the link therefore has to be re-read with `overrideAccess` (`resolveOwnMemberId`, the same move `/app`'s viewer makes); and the *time* rule ("only before the performance starts") is deliberately not in the `Where` — it cannot be expressed against a relation without a join, so it lives in the answer route's pure rules, which is the only writer the app uses.

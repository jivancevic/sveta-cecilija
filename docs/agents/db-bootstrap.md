...
**Dropping `required` or `defaultValue` from a Payload field IS a schema change.** The db-postgres adapter derives `NOT NULL` from `required` and the column `DEFAULT` from `defaultValue`, so a field-config-only edit still moves the drift gate. Verified on `Users.role` (#397): push emitted a bare `role public.enum_users_role`, one drifting line against `00-base.sql`, fixed with an `ALTER COLUMN … DROP DEFAULT` / `DROP NOT NULL` migration plus the matching base-schema edit. Reproduce the gate locally the way CI does: two throwaway DBs on one server, `next dev` into one (Payload push) and `bootstrap-db.mjs` into the other, `pg_dump --schema-only` both, `node scripts/schema-diff.mjs expected.sql bootstrap.sql`. Point `.env.local` at the throwaway DB rather than the shell env, because Next 16 loads `.env.local` over it.

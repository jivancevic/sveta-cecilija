...
## A one-off data script uses `pg`, and its rules live in a vitest test

There is **no working route from a script in `scripts/` to the Payload local API** in this repo, so don't spend a session rediscovering that. `payload run x.ts` transpiles to CJS (top-level `await` is a build error), swallows CLI flags — `process.argv.slice(2)` arrives empty, so a `--dry-run` guard never fires — and kills the process on `await import('payload')` with **exit code 0 and no output**, which reads as success. Running `tsx` directly instead dies inside Payload's own `dist/bin/loadEnv.js`. Both fail identically on Node 22 and 25. The `seed:faqs` / `seed:posts` npm scripts are wired this way and fail the same way, so their presence in `package.json` is not evidence the route works.

Write a plain `.mjs` on `pg`, like `seed-dev-moreskanti.mjs`, `backfill-offline-sales-2026.mjs` and every `probe-*.mjs`. That buys correctness at the cost of the collection hooks: **raw SQL bypasses `beforeValidate`**, so a script can write a row the admin UI would have rejected. Put the rules back by exporting the data from the script and feeding every row to the real pure validator from `src/lib/` in a vitest test — it runs in CI, and the vocabulary is never re-typed. Worked pair: `scripts/import-moreskanti-2026.mjs` + `src/lib/roster-2026.test.ts`.

Reaching production from such a script: the prod Postgres container publishes no host port, so `docker cp` the `.mjs` into the **app** container (it already has node, `pg` and the right `DATABASE_URL`) and `docker exec` it there.
